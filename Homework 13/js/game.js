...
//random numbers for initial x,y,diameters for enemies.
    for(var i = 0; i < 5; i++)
    {
        enemyX[i] = getRandomNumber(500);
        enemyY[i] = getRandomNumber(600);
        enemyDiameters[i] = getRandomNumber(100);
    }
}
function draw()
{
...
//    text("Y: " + mouseY,100,220 );
    ...
    winCondition();
    exit();
//5 random enemies. or powerups.
    for(var i = 0; i < enemyX.length; i++)
    {
        enemycircle(enemyX[i],enemyY[i],enemyDiameters[i]/2,35,120);
    }
}
//exit box
function exit()
...
//mouseClicked for placing shape.
...
//set maze
//going to add to array. maybe use line() instead when we do  collision
function maze1()
{
    fill(0);
    stroke(4);
...
*/
//create random enemies function
function enemycircle(x,y,diameter)
{
    fill('white');
    circle(x,y,diameter);
}
